...
public class ProductServiceImpTest {
	...
	public void testCreate() {
		...
		Mockito.when(productDao.create(Mockito.any(Product.class))).thenReturn(Boolean.TRUE);
		Boolean rst = productServiceImpl.create(pGet);
		assertTrue(rst);
	}

	@Test
	public void testUpdate() {
		Mockito.when(productDao.update(Mockito.any(Product.class))).thenReturn(Boolean.TRUE);
		Boolean rst = productServiceImpl.update(pGet);
		assertTrue(rst);
	}

	@Test
	public void testDelete() {
		Mockito.when(productDao.delete(Mockito.anyInt())).thenReturn(Boolean.TRUE);
		Boolean rst = productServiceImpl.delete(1);
		assertTrue(rst);
	}
	...
	public void testAssociateMarchant() {
		Mockito.when(productDao.associateMarchant(Mockito.any(Product.class),Mockito.any(Marchant.class))).thenReturn(Boolean.TRUE);
		Boolean rst = productServiceImpl.associateMarchant(pGet,mAssoc);
		assertTrue(rst);
